<script lang="ts">
	...
	const actionButtonBase =
		'inline-flex cursor-pointer items-center justify-center gap-2 rounded-full px-5 py-2 text-sm font-medium uppercase tracking-[0.2em] transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white/70 disabled:cursor-not-allowed disabled:opacity-60';
</script>

<Dialog.Root bind:open>
	<Dialog.Trigger
		class="group inline-flex cursor-pointer items-center gap-3 rounded-full border border-white/30 bg-white/10 px-6 py-3 text-sm font-semibold tracking-[0.2em] text-white/80 uppercase shadow-[0_14px_28px_-18px_rgba(0,0,0,0.6)] transition hover:border-white/60 hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white/70"
	>
		{label}
	</Dialog.Trigger>
	<Dialog.Portal>
			...
			<div class="flex flex-col gap-6">
				<header class="space-y-2">
					<Dialog.Title class="text-xl font-semibold text-foreground"
						>{m.submit_film_dialog_title()}</Dialog.Title
					>
				...
				>
					<label class="space-y-2">
						...
						<input
							...
							class="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-base text-white shadow-inner placeholder:text-white/50 focus:border-[#e50914] focus:ring-2 focus:ring-[#e50914]/70 focus:outline-none"
						/>
					</label>
					{#if submitError}
