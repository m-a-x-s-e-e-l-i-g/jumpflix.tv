...
export function parseTrackCandidatesFromMusicInThisVideo(
	initialData: any
...
export async function fetchYouTubeDescription(videoId: string): Promise<string> {
	...
	const altDesc =
		data?.engagementPanels?.[0]?.engagementPanelSectionListRenderer?.content?.structuredDescriptionContentRenderer?.items?.[0]?.videoDescriptionHeaderRenderer?.description?.runs
			?.map((r: any) => r?.text)
			.join('');
