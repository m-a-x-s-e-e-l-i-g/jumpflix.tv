<script lang="ts">
  ...
  const FACET_ENVIRONMENT_OPTIONS: FacetEnvironment[] = ['street', 'rooftops', 'nature', 'urbex', 'gym'];
  ...
  $effect(() => {
    ...
    if (selected?.type === 'series') {
      ...
      if (typeof epPos === 'number' && Number.isFinite(epPos) && typeof sn === 'number' && Number.isFinite(sn)) {
        seasonNumber = sn;
  ...
  let descriptionEdited: boolean = $derived(normString(description) !== normString(initial?.description));
  ...
  let externalUrlEdited: boolean = $derived(normString(externalUrl) !== normString(initial?.externalUrl));
  ...
  let facetTypeEdited: boolean = $derived(normString(setFacetType) !== normString(initial?.facetType));
  let facetEnvironmentEdited: boolean = $derived(normString(setFacetEnvironment) !== normString(initial?.facetEnvironment));
  let facetFilmStyleEdited: boolean = $derived(normString(setFacetFilmStyle) !== normString(initial?.facetFilmStyle));
  let facetThemeEdited: boolean = $derived(normString(setFacetTheme) !== normString(initial?.facetTheme));
  let facetMoodEdited: boolean = $derived(!stringSetEqual(facetMood as unknown as string[], initial?.facetMood ?? []));
  let facetMovementEdited: boolean = $derived(!stringSetEqual(facetMovement as unknown as string[], initial?.facetMovement ?? []));
  ...
  $effect(() => {
    ...
    const selectedTrack = list.find((t: any) => normString(t?.song?.spotifyUrl) === currentKey) ?? list[0];
  ...
  function buildPayload(): any {
    ...
    if (kind === 'poster' && (posterEdited || !initial) && (posterEdited || normString(posterUrl))) {
    ...
    if (kind === 'subscription' && subscriptionEdited) {
      ...
      if (normString(provider) !== normString(initial?.provider)) set.provider = normString(provider) || null;
      if (normString(externalUrl) !== normString(initial?.externalUrl)) set.external_url = normString(externalUrl) || null;
    ...
    if (kind === 'tracks') {
      ...
      if (trackAction === 'edit') {
        const t = (selected?.tracks ?? []).find((x: any) => normString(x?.song?.spotifyUrl) === normString(effectiveSpotifyUrl));
      ...
      <div class="flex flex-col gap-6">
        <header class="space-y-2">
          ...
          <p id="content-suggestion-description" class="text-sm text-muted-foreground">
            Suggest edits (facets/people/tracks), report broken links, or flag new episodes/seasons.
          </p>
          {#if selected}
            <div class="text-xs text-white/70">Target: <span class="text-white">{selected.title}</span></div>
          {/if}
        </header>
        ...
        <div class="grid grid-cols-1 gap-4">
          <label class="space-y-2">
            ...
            <select
              bind:value={kind}
              class={editedSelectClass(false)}
            >
              <option value="facets" class="bg-background text-foreground">Facets</option>
              <option value="people" class="bg-background text-foreground">Creators / starring (athletes)</option>
              <option value="description" class="bg-background text-foreground">Description</option>
              {#if selected?.type === 'series'}
                <option value="new_episode" class="bg-background text-foreground">New episode available</option>
                <option value="new_season" class="bg-background text-foreground">New season available</option>
              {/if}
              <option value="tracks" class="bg-background text-foreground">Tracks & timestamps</option>
              <option value="broken_link" class="bg-background text-foreground">Content not available / broken link</option>
              <option value="subscription" class="bg-background text-foreground">Subscription / provider change</option>
              <option value="poster" class="bg-background text-foreground">Different poster</option>
            </select>
          </label>

          {#if selected?.type === 'series'}
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <div class="text-sm font-medium text-white/80 mb-2">Applies to</div>
          ...
          {#if kind === 'poster'}
            <label class="space-y-2">
              <span class="text-sm font-medium text-white/80 inline-flex items-center gap-2">
                Poster URL
              ...
              <input
                ...
                placeholder="https://..."
                class={
                  `w-full rounded-xl border bg-white/10 px-4 py-3 text-base text-white shadow-inner placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-[#e50914]/70 ${
                    posterEdited ? 'border-[#e50914]' : 'border-white/20 focus:border-[#e50914]'
                  }`
                }
              />
              {#if initial?.posterUrl}
                <div class="text-xs text-white/60">Current: <span class="text-white/80">{initial.posterUrl}</span></div>
              {/if}
            </label>
          {/if}
          ...
          {#if kind === 'subscription'}
            <div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              ...
              <label class="space-y-1">
                <span class="text-xs text-white/70 inline-flex items-center gap-2">
                  Provider
            ...
            <div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              <label class="space-y-2">
                ...
                <textarea
                  ...
                  class={
                    `w-full rounded-xl border bg-white/10 px-4 py-3 text-sm text-white shadow-inner placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-[#e50914]/70 ${
                      descriptionEdited ? 'border-[#e50914]' : 'border-white/20 focus:border-[#e50914]'
                    }`
                  }
                ></textarea>
                {#if initial?.description}
                  <div class="text-xs text-white/60">Current: <span class="text-white/80">{initial.description}</span></div>
                {/if}
              </label>
            </div>
          {/if}
          ...
          {#if kind === 'people'}
            <div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              <div class="space-y-2">
                <div class="text-sm font-medium text-white/80 inline-flex items-center gap-2">
                  Creators
                      ...
                      <button
                        ...
                        class={
                          `inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs ${
                            (initial?.creators ?? []).some((v) => v.toLowerCase() === c.toLowerCase())
                  ...
                  <div class="flex flex-wrap gap-2">
                    {#each starring as s (s)}
                      <button
                        ...
                        class={
                          `inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs ${
                            (initial?.starring ?? []).some((v) => v.toLowerCase() === s.toLowerCase())
                              ? 'border-white/15 bg-white/5 text-white/80'
                              : 'border-[#e50914]/30 bg-[#e50914]/10 text-[#ff9aa0]'
                          }`
                        }
            ...
            <div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <label class="space-y-1">
                  ...
                  <select bind:value={setFacetEnvironment} class={editedSelectClass(facetEnvironmentEdited)}>
                    <option value="" class="bg-background text-foreground">(none)</option>
                ...
                <label class="space-y-1">
                  ...
                  <select bind:value={setFacetFilmStyle} class={editedSelectClass(facetFilmStyleEdited)}>
                    <option value="" class="bg-background text-foreground">(none)</option>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="space-y-2">
                  <div class="text-xs text-white/70 inline-flex items-center gap-2">
                    Mood
                    <span class={editedBadge(facetMoodEdited)}>Edited</span>
                  </div>
                  <div class={
                    `rounded-lg border p-3 ${facetMoodEdited ? 'border-[#e50914]/40 bg-[#e50914]/5' : 'border-white/10 bg-white/5'}`
                  }>
                    <div class="flex flex-wrap gap-3 text-sm text-white/80">
                ...
                <div class="space-y-2">
                  ...
                  <div class={
                    `rounded-lg border p-3 ${facetMovementEdited ? 'border-[#e50914]/40 bg-[#e50914]/5' : 'border-white/10 bg-white/5'}`
                  }>
                    <div class="flex flex-wrap gap-3 text-sm text-white/80">
          ...
          {#if kind === 'new_season'}
            <div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              <label class="space-y-1">
                <span class="text-xs text-white/70">Season #</span>
                <input type="number" min="1" bind:value={seasonNumber} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
              </label>
              <label class="space-y-1">
                <span class="text-xs text-white/70">YouTube playlist id (optional)</span>
                <input bind:value={newSeasonPlaylistId} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
              </label>
              <label class="space-y-1">
                <span class="text-xs text-white/70">Custom season name (optional)</span>
                <input bind:value={newSeasonCustomName} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
              </label>
            </div>
          {/if}
          ...
          {#if kind === 'new_episode'}
            <div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              <div class="grid grid-cols-2 gap-3">
                ...
                <label class="space-y-1">
                  <span class="text-xs text-white/70">Episode #</span>
                  <input type="number" min="1" bind:value={episodeNumber} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
                </label>
              </div>
              <label class="space-y-1">
                <span class="text-xs text-white/70">Video ID (YouTube/Vimeo) (optional)</span>
                <input bind:value={newEpisodeVideoId} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
              </label>
              <label class="space-y-1">
                <span class="text-xs text-white/70">Episode title (optional)</span>
                <input bind:value={newEpisodeTitle} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
              </label>
              <label class="space-y-1">
                <span class="text-xs text-white/70">Thumbnail URL (optional)</span>
                <input bind:value={newEpisodeThumbnail} class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#e50914] focus:outline-none" />
              </label>
            </div>
          {/if}
                  ...
                  <div class="space-y-2">
                    ...
                    <div class="max-h-56 overflow-auto rounded-lg border border-white/10 bg-black/20">
                      {#each selected.tracks as t (t.song.spotifyTrackId)}
                        <label class="flex items-start gap-3 px-3 py-2 border-b border-white/5 last:border-b-0 hover:bg-white/5">
                          <input
                            class="mt-1"
                            type="radio"
                    ...
                    <div class="text-xs text-white/60">Selected: <span class="text-white/80">{trackRemoveSpotifyUrl || '—'}</span></div>
                  </div>
                {:else}
                  <div class="text-xs text-white/60">No tracks found for this video. You can still paste a Spotify URL below.</div>
                  <label class="space-y-1">
              ...
              {:else if trackAction === 'edit'}
                {#if selected?.tracks?.length}
                  <div class="space-y-2">
                    ...
                    <div class="text-xs text-white/60">Selected: <span class="text-white/80">{trackEditSpotifyUrl || '—'}</span></div>
                  </div>
              ...
              {#if trackAction === 'add'}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label class="space-y-1">
                    ...
                    <input bind:value={trackArtist} class={editedInputClass(Boolean(trackArtist.trim()))} />
                  </label>
                  <label class="space-y-1">
                    <span class="text-xs text-white/70">Title</span>
                    <input bind:value={trackTitle} class={editedInputClass(Boolean(trackTitle.trim()))} />
                  </label>
                  <label class="space-y-1">
                    <span class="text-xs text-white/70">Start timecode (mm:ss)</span>
                    <input bind:value={trackStartTimecode} class={editedInputClass(Boolean(trackStartTimecode.trim()))} />
        ...
        <div class="mt-2 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3">
          <button
            type="button"
            class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-full px-5 py-2 text-sm font-medium uppercase tracking-[0.2em] transition border border-white/20 bg-white/5 text-white/70 hover:border-white/40 hover:text-white"
            onclick={() => (open = false)}
          >
            Cancel
          </button>
          <button
            ...
            class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-full px-5 py-2 text-sm font-medium uppercase tracking-[0.2em] transition bg-[#e50914] text-white shadow-[0_12px_32px_-18px_rgba(229,9,20,0.9)] hover:bg-[#ff1a27] disabled:bg-[#e50914]/80 disabled:opacity-70"
            onclick={submit}
          >
