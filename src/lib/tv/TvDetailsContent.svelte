<script lang="ts">
	...
	$: {
		if (selected?.type === 'series') {
			const season = (selected as any).seasons?.find(
				(s: any) => s.seasonNumber === selectedSeasonNum
			);
			if (season) {
	...
	$: if (browser && currentFetchId) {
		...
		fetch(`/api/series/${encodeURIComponent(fetchId)}/episodes`, {
	...
	$: if (browser && episodes && episodes.length > 0) {
		...
		const hasEpisodeInPath =
			$page.url.pathname.includes('/seasons/') && $page.url.pathname.includes('/episodes/');
						...
						{#if selected?.type === 'series'}
							{#if selectedEpisode}
								...
								{:else}
									{m.tv_noPlayer()}
								{/if}
							{:else if seriesExternalSourceUrl}
								{m.tv_watchOn()}
								{selected?.provider || ((selected as any)?.trakt ? 'Trakt' : 'External')}
							{:else}
								Play series
			...
			<div class="detail-meta">
				...
				<div class="detail-meta-actions">
					{#if (selected as any).trakt}
						<a
							href={withUtm((selected as any).trakt)}
							target="_blank"
					...
					{#if providerLink}
						...
						>
							{#if providerLink.kind === 'youtube'}
								<span class="provider-icon provider-icon-youtube h-4 w-4" aria-hidden="true"></span>
							{:else}
									...
									<select
										...
										on:change={(e) => {
											...
											Promise.resolve().then(() =>
												onSelectEpisode('pos:1', 'Episode 1', 1, seasonForUrl)
											);
											Promise.resolve().then(() => {
												try {
										...
										<li>
												...
												<div class="detail-episode-thumb">
													{#if ep.thumbnail}
														<img
															src={ep.thumbnail}
															alt={decode(ep.title)}
															class={epProgress?.isWatched ? 'is-watched' : ''}
															loading="lazy"
															decoding="async"
														/>
													{:else}
													...
													{#if epProgress?.isWatched}
														<div class="detail-episode-status"><CheckIcon class="h-4 w-4" /></div>
													{/if}
											...
											{#if isAuthenticated}
												<button
													...
													aria-label={epProgress?.isWatched
														? 'Mark as unwatched'
														: 'Mark as watched'}
												>
													{#if epProgress?.isWatched}
