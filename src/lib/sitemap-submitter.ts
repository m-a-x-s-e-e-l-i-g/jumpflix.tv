	...
	async submitToGoogle(): Promise<{ success: boolean; message: string }> {
		...
		try {
			...
			if (response.ok) {
				...
				return {
					success: false,
					message: `Google submission returned status: ${response.status}. Consider using Google Search Console API for reliable submissions.`
				};
			}
		} catch (error) {
			return {
				success: false,
				message: `Google submission error: ${error}. Consider using Google Search Console API for reliable submissions.`
			};
		}
	}
	...
	async submitToBing(): Promise<{ success: boolean; message: string }> {
		...
		try {
			...
			if (response.ok) {
				...
				return {
					success: false,
					message: `Bing submission returned status: ${response.status}. Consider using Bing Webmaster Tools API for reliable submissions.`
				};
			}
		} catch (error) {
			return {
				success: false,
				message: `Bing submission error: ${error}. Consider using Bing Webmaster Tools API for reliable submissions.`
			};
		}
	}
