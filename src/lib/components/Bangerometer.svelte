...
<div
	class="banger-meter-container {shake ? 'shake' : ''} {screenShake ? 'screen-shake' : ''}"
	class:authenticated={isAuthenticated}
	class:has-rating={hasUserRating}
	class:collapsed={!isExpanded}
	class:epic-mode={rating >= 10}
>
		...
		>
			<div class="collapsed-shell">
				...
				<div class="collapsed-meta">
					...
					{:else}
						<span class="collapsed-empty text-sm text-gray-400">No ratings yet</span>
					{/if}
					{#if isAuthenticated}
						<svg
							class="collapsed-indicator h-4 w-4 text-gray-400"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
		...
		<div class="expanded-view">
			<div class="mb-3 flex items-center justify-between">
				<div class="flex items-center gap-2">
					...
					{#if hasUserRating && isAuthenticated}
						<span class="text-xs font-medium text-orange-400">(Your rating)</span>
					{/if}
				</div>
				<div class="flex items-center gap-2">
					{#if rating > 0}
			...
			>
				...
				<div class="slider-track">
					...
					<div
						class="slider-fill bg-gradient-to-r {getGradientColor(
							intensity
						)} transition-all duration-300"
						style:width="{intensity * 100}%"
						style:box-shadow={rating >= 9
						...
						{#if rating >= 7}
							<div class="flames-container">
								{#each Array(Math.floor(rating / 2.5)) as _, i}
									...
									>
										<FlameIcon
											class="w-{rating >= 10 ? '6' : '4'} h-{rating >= 10
												? '6'
												: '4'} drop-shadow-glow text-yellow-300"
										/>
									</div>
								{/each}
							</div>
						...
						{#if rating >= 9}
							<div class="lightning-container">
								{#each Array(rating === 10 ? 3 : 1) as _, i}
									...
									>
										<ZapIcon class="h-5 w-5 text-yellow-200" />
									</div>
								{/each}
							</div>
						{/if}
					...
					{#each explosionParticles as particle (particle.id)}
						...
						>
							{#if rating >= 10}
								<SparklesIcon class="h-5 w-5 animate-spin text-yellow-300" />
							{:else if rating >= 8}
								<ZapIcon class="h-4 w-4 text-orange-400" />
							{:else}
								<div
									class="h-3 w-3 rounded-full bg-gradient-to-br from-orange-400 to-red-500"
								></div>
							{/if}
						</div>
					{/each}
			...
			<div class="mt-3 text-center">
				<p
					class="text-sm font-medium {rating >= 9
						? 'animate-pulse font-bold text-orange-400'
						: 'text-gray-400'} transition-colors {rating === 10 ? 'text-lg' : ''}"
				>
					{#if !isAuthenticated}
						Sign in to rate
					{:else if !isWatched}
...
<style>
	...
	.mega-flame {
		animation:
			megaFlicker 0.3s ease-in-out infinite alternate,
			floatUp 1.5s ease-in-out infinite;
	}
