<script lang="ts">
  ...
  import NetflixHero from '$lib/tv/NetflixHero.svelte';
  import NetflixNav from '$lib/tv/NetflixNav.svelte';
  import TvSearchControls from '$lib/tv/TvSearchControls.svelte';
  import NetflixCatalog from '$lib/tv/NetflixCatalog.svelte';
  ...
  // Get featured item for Netflix hero
  $: featuredItem = $visibleContent.length > 0 ? $visibleContent[0] : null;
  
  // Get admin status from page data
  $: isAdmin = ($page?.data as any)?.isAdmin ?? false;
  
  function handleSearchClick() {
    showSearchSection = !showSearchSection;
    if (showSearchSection && browser) {
      setTimeout(() => {
  ...
  function handleHeroPlay(item: ContentItem) {
    handleOpenContent(item);
  }
  
  function handleHeroInfo(item: ContentItem) {
    handleSelect(item);
  }
  ...
  onMount(() => {
    ...
    // Track scroll for nav bar background
    const handleScroll = () => {
      if (!browser) return;
...
<div class="relative isolate min-h-screen bg-background text-foreground tv-page overflow-x-hidden md:pr-[420px] xl:pr-[460px]">
  ...
  <!-- Netflix-style Catalog with Carousels -->
  <div class="catalog-section relative z-20 -mt-32" style:min-height={catalogMinHeight ? `${catalogMinHeight}px` : undefined}>
    <NetflixCatalog
