...
// Remember string preferences across sessions on the same device.
function loadStringPreference(key: string, defaultValue = ''): string {
	if (!browser) return defaultValue;
	try {
		const raw = localStorage.getItem(key);
...
function persistStringPreference(key: string, value: string) {
	if (!browser) return;
	try {
...
function refreshProgressSets() {
	...
	const progressSummary = new Map<
		string,
		{ tracked: number; watchedCount: number; hasPartial: boolean }
	>();
	...
	for (const entry of entries) {
		...
		const summary = progressSummary.get(baseId) ?? {
			tracked: 0,
			watchedCount: 0,
			hasPartial: false
		};
...
export function selectContent(item: ContentItem) {
	...
	const unsub = visibleContent.subscribe((list) => {
		currentList = list;
	});
	unsub();
...
export function setSort(value: SortBy) {
	sortBy.set(value);
}
