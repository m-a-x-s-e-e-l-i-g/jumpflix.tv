...
async function submitToGoogle() {
	try {
		...
		if (response.ok) {
			...
			console.log(
				'ℹ️  Note: For reliable Google submissions, consider using Google Search Console API'
			);
		...
		console.log(
			'ℹ️  For reliable Google submissions, use Google Search Console API or manual submission'
		);
		return false;
	}
}
...
async function submitToBing() {
	try {
		...
		if (response.ok) {
			...
			console.log(
				'ℹ️  Note: For reliable Bing submissions, consider using Bing Webmaster Tools API'
			);
		...
		console.log(
			'ℹ️  For reliable Bing submissions, use Bing Webmaster Tools API or manual submission'
		);
		return false;
	}
}
...
async function main() {
	...
	if (isLocalhostUrl(SITE_URL) || isLocalhostUrl(SITEMAP_URL)) {
		if (process.env.FORCE_SITEMAP_SUBMISSION) {
			console.log(
				'⚠️  PUBLIC_SITE_URL points to localhost but FORCE_SITEMAP_SUBMISSION is set. Proceeding anyway.'
			);
		} else {
