<script lang="ts">
	...
	// Track whether overlay was open before switching to desktop (for rotation handling)
	...
	onMount(() => {
		...
		function updateIsMobile() {
			...
			if (wasMobile && !isMobile) {
				// Switching from mobile to desktop: close overlay and remember it was open
				wasOverlayOpen = get(showDetailsPanel);
				showDetailsPanel.set(false);
			} else if (!wasMobile && isMobile) {
				// Switching from desktop to mobile: restore overlay if it was open and there's a selected item
				if (wasOverlayOpen && get(selectedContent)) {
