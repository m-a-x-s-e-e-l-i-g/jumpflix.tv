...
export function filterAndSortContent(
	...
	switch (state.sortBy) {
		...
		case 'duration-asc':
			sorted.sort(
				(a, b) =>
					parseDurationToMinutes((a as any).duration) - parseDurationToMinutes((b as any).duration)
			);
			break;
		case 'duration-desc':
			sorted.sort(
				(a, b) =>
					parseDurationToMinutes((b as any).duration) - parseDurationToMinutes((a as any).duration)
			);
			break;
		default:
