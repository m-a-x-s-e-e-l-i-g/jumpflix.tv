...
async function main() {
  ...
  for (const file of files) {
    try {
      ...
      if (existing[key] && existing[key] !== hash) updated += 1; else processed += 1;
      map[key] = hash;
    } catch (e) {
  ...
  const entries = sortedKeys.map((k) => `  ${JSON.stringify(k)}: ${JSON.stringify(map[k])},`).join('\n');
  const content = `// AUTO-GENERATED by scripts/generate-blurhash.mjs. Do not edit by hand.\n` +
