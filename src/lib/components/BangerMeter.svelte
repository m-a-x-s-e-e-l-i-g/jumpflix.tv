<script lang="ts">
  ...
  let shake = false;
  let explosionParticles: Array<{ id: number; x: number; y: number; delay: number }> = [];
  let particleId = 0;
  ...
  function triggerEffects(x: number, width: number) {
    ...
    if (rating >= 7) {
      const particleCount = rating >= 10 ? 5 : 3;
      ...
      setTimeout(() => {
        explosionParticles = explosionParticles.slice(-10);
      }, 1000);
    }
  }
...
<div class="banger-meter-container {shake ? 'shake' : ''}" class:authenticated={isAuthenticated} class:has-rating={hasUserRating} class:collapsed={!isExpanded}>
    ...
    >
      <div class="flex items-center justify-between w-full">
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-sm font-semibold text-gray-200">Banger Meter</h3>
        </div>
    ...
    <div class="slider-track">
      ...
      >
        ...
        {#if rating >= 7}
          <div class="flames-container">
            {#each Array(Math.floor(rating / 3)) as _, i}
              <div
                class="flame"
                style:left="{(i + 1) * 25}%"
                transition:scale={{ duration: 300 }}
              >
                <FlameIcon class="w-4 h-4 text-yellow-300 drop-shadow-glow" />
              </div>
            {/each}
          </div>
        {/if}
      ...
      {#each explosionParticles as particle (particle.id)}
        ...
        >
          ...
          {:else if rating >= 8}
            <ZapIcon class="w-3 h-3 text-orange-400" />
          {:else}
  ...
  <div class="mt-3 text-center">
    <p class="text-sm font-medium {rating >= 9 ? 'text-orange-400' : 'text-gray-400'} transition-colors">
      {#if !isAuthenticated}
        Sign in to rate
      {:else if !isWatched}
...
<style>
  ...
  @keyframes flicker {
    ...
    100% { transform: translateY(-2px) scale(1.1); opacity: 1; }
  }
